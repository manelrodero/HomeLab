...
function Write-Registry {
    ...
    Write-Log ('Writing registry from file {0}' -f $tempFile) -Level Info -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
    ...
    if ($exitCode -ne 0) {
        Write-Log 'There was an error merging the reg file' -Level Warning -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
    } else {
...
if (-not (Test-Path -Path $RegFile)) {
    Write-Log "RegFile $RegFile doesn't exist. Operation aborted" -Level Error -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
} else {

    if ($CurrentUser -or $AllUsers -or $DefaultProfile) {

        Write-Log ('Reading the registry file {0}' -f $RegFile) -Level Info -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
        $registryData = Get-Content -Path $RegFile -ReadCount 0

        if ($CurrentUser) {
            Write-Log "Writing to the currenlty logged on user's registry" -Level Info -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
        ...
        if ($AllUsers) {
            Write-Log "Writing to every user's registry" -Level Info -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
            ...
            $res -notmatch 'S-1-5-18|S-1-5-19|S-1-5-20|DEFAULT|Classes' | ForEach-Object {
                if ($_ -ne '') {
                    ...
                    $RegFileContents = $registryData -replace 'HKEY_CURRENT_USER', "HKEY_USERS\$sid"
                    Write-Log "- $sid" -Level Info -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
                    Write-Registry -RegFileContents $RegFileContents
                }
            }
        }
        ...
        if ($DefaultProfile) {
            Write-Log "Writing to the default profile's registry (for future users)" -Level Info -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
        ...
        Write-Log 'No mode was selected. Operation aborted' -Level Warning -UseCMTrace -LogName "LogName" -Component "WriteToHKCUFromSystemWithLogs"
    }
}
