# HomeLab

Este repositorio contiene información sobre la configuración de mi _HomeLab_ con la idea de poder compartirlo con amigos y compañeros que necesiten ejecutar servicios similares.

La idea es que esta información sea lo más neutra posible pero, al mismo tiempo, que pueda servir para recuperar desde cero mi entorno en caso de desastre.

Por este motivo, la información será muy esquemática o "paso a paso" simples que se puedan seguir fácilmente si se tiene un mínimo conocimiento del sistema operativo, tecnología o servicio que se describe.

Si tengo información más detallada en algún artículo de [mi blog personal](https://www.manelrodero.com/blog/) se hará referencia a la misma para poder ampliar o completar la información.

## Servidores

Mi pequeño laboratorio en casa está formado por los tres "**servidores**" siguientes:

* **Raspberry Pi 4** ejecutando **Raspberry Pi OS Lite**
* **Dell OptiPlex 7050 Micro** ejecutando **Proxmox Virtual Environment**
* **PC clónico** con formato torre ejecutando **Unraid**

## Descargo de responsabilidad (_Disclaimer_)

El contenido de este repositorio —incluyendo código, scripts, configuraciones, documentación y cualquier otro recurso— se proporciona **únicamente con fines informativos y educativos**. Todo el material se ofrece **“tal cual”**, sin garantías de ningún tipo, explícitas o implícitas.

El uso que hagas de esta información es **exclusivamente tu responsabilidad**. Antes de aplicar cualquier cambio en tu entorno, especialmente en sistemas de producción o infraestructuras críticas, asegúrate de:

- Revisar y comprender completamente el funcionamiento del código o configuración.  
- Adaptarlo a tus necesidades y a tu contexto técnico.  
- Probarlo en entornos controlados o de pruebas.  

El autor **no se hace responsable** de daños, pérdidas de datos, interrupciones de servicio, problemas de seguridad o cualquier otro perjuicio derivado del uso, mal uso o interpretación del contenido de este repositorio.

Si decides utilizar cualquier parte de este proyecto, lo haces bajo tu propio criterio y riesgo.
